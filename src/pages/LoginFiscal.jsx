...
export default function LoginFiscal() {
  ...
  const handleLogin = async (e) => {
    ...
    setErro('')
    try {
      const { data } = await api.post('/auth/login-fiscal', { email, cpf })
  ...
  return (
    <div className="bg-white p-8 rounded-2xl shadow-2xl max-w-sm w-full">
      ...
      <form onSubmit={handleLogin} className="flex flex-col gap-4">
        ...
        <div><label className="block text-xs font-medium text-slate-500 mb-1">CPF</label><input type="text" placeholder="000.000.000-00" className="w-full border p-2 rounded" value={cpf} onChange={(e) => setCpf(e.target.value)} required /></div>
